...
/** Build the standardized issue title for a finding. */
export function buildIssueTitle(finding: Finding): string {
	return `[ai-review] ${finding.category}: ${finding.title} (${finding.file}:${finding.line})`;
}
...
/**
 * Create a GitHub issue for a verified finding.
 * Runs `gh issue create` on the host machine.
 * Returns the issue URL, or undefined on failure.
 */
export function createGitHubIssue(repo: string, finding: Finding): string | undefined {
